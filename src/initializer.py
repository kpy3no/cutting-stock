...
class FigureInitializer:
    def __init__(self, count=100, a_width=6, a_height=5, varians=2, space_max_x=1000, space_max_y=1000, max_iterations=100000):
    ...
    def generate_rectangles(self):
        ...
        for width, height in zip(widths, heights):
            if width < 1:
                width = 1
            elif width >= self.space_max_x:
                width = self.space_max_x - 1

            if height < 1:
                height = 1
            elif height >= self.space_max_y:
                height = self.space_max_y - 1
    ...
    def insert_figure_random(self, width, height):
        for x in range(self.max_iterations):
            start_point = Point(np.random.randint(0, self.space_max_x - width),
...
def doOverlap(l1, r1, l2, r2):
    ...
    if (l2.x >= l1.x and r2.x <= r1.x and l2.y >= l1.y and r2.y <= r1.y)\
            or (l1.x >= l2.x and r1.x <= r2.x and l1.y >= l2.y and r1.y <= r2.y):
        return False
